-- ============================================================
-- Migration 00023: Apply Missing Audit RPCs
-- ============================================================
-- This migration creates all audit system objects that were
-- defined in 00021/00022 but never applied to the database.
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('actions', jsonb_object_agg(action, cnt))
  FROM (
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('users', jsonb_agg(jsonb_build_object('user_id', user_id, 'email', email, 'name', name, 'actions', cnt)))
  FROM (
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('attempts', jsonb_agg(jsonb_build_object('email', email, 'ip', ip_address, 'time', attempted_at)))
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('users', jsonb_agg(jsonb_build_object('user_id', user_id, 'ip_count', ip_count)))
  FROM (
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('changes', jsonb_agg(jsonb_build_object('action', action, 'details', details, 'created_at', created_at)))
  FROM public.activity_logs
  ...
  SELECT
    ...
    COUNT(*)::BIGINT,
    jsonb_build_object('events', jsonb_agg(jsonb_build_object('action', action, 'details', details, 'created_at', created_at)))
  FROM public.activity_logs
